...
class DWConv(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        B, N, C = x.shape
        n = N // 21
        x1 = x[:, 0:16 * n, :].transpose(1, 2).view(B, C, H * 4, W * 4).contiguous()
        x2 = x[:, 16 * n:20 * n, :].transpose(1, 2).view(B, C, H * 2, W * 2).contiguous()
        x3 = x[:, 20 * n:, :].transpose(1, 2).view(B, C, H, W).contiguous()
