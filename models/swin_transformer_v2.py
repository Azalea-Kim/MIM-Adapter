...
# The code is from Swin Transformer V2.
...
from .checkpoint import load_checkpoint_swin
from .checkpoint import get_root_logger

from .adapter import SpatialPriorModule, deform_inputs, Injector, Extractor
...
class SwinTransformerV2(nn.Module):
    ...
    def __init__(self,
        ...
        for i_layer in out_indices[:self.num_layers]:
            layer = norm_layer(num_features[i_layer])
            layer_name = f'norm{i_layer}'
            self.add_module(layer_name, layer)

        self.injector = Injector(dim=128, n_levels=3, num_heads=8, init_values=0.,
                                 n_points=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), deform_ratio=1.0)
        self.extractor = Extractor(dim=1024, n_levels=1, num_heads=8, n_points=4,
                                   norm_layer=partial(nn.LayerNorm, eps=1e-6), deform_ratio=1.0, with_cffn=True,
                                   cffn_ratio=0.25)
        
        self.extra_extractors = nn.Sequential(*[
            Extractor(dim=1024, n_levels=1, num_heads=8, n_points=4,
                                   norm_layer=partial(nn.LayerNorm, eps=1e-6), deform_ratio=1.0, with_cffn=True,
                                   cffn_ratio=0.25)
            for _ in range(2)
        ])
        self.up_dim = nn.Linear(128, 1024)
        self.up = nn.ConvTranspose2d(1024, 1024, 2, 2)
        self.spm = SpatialPriorModule(inplanes=64, embed_dim=128)
        self.spm.apply(self._init_weights2)
        self.injector.apply(self._init_weights2)
        self.extractor.apply(self._init_weights2)
        self.up_dim.apply(self._init_weights2)
        self.up.apply(self._init_weights2)
        self.extra_extractors.apply(self._init_weights2)
        ...
        for i in range(self.num_layers):
            ...
            if i in self.out_indices:
                ...
                c = self.extractor(query=c, reference_points=deform_inputs2[0],
                           ...
                           level_start_index=deform_inputs2[2], H=Wh, W=Ww)
                ...
                bs, dim = c.size(0), c.size(2)

                c2 = c2.transpose(1, 2).view(bs, dim, H * 4, W * 4).contiguous()
                c3 = c3.transpose(1, 2).view(bs, dim, H * 2, W * 2).contiguous()
                c4 = c4.transpose(1, 2).view(bs, dim, H, W).contiguous()
                # c1 = self.up(c2) + c1
                # c1 = F.interpolate(c1, scale_factor=8, mode='bilinear', align_corners=False)
                c2 = F.interpolate(c2, scale_factor=0.25, mode='bilinear', align_corners=False).flatten(2).permute(0,2,1)
                c3 = F.interpolate(c3, scale_factor=0.5, mode='bilinear', align_corners=False).flatten(2).permute(0,2,1)
                c4 = c4.flatten(2).permute(0,2,1)
                x_out = x_out + c2 + c3 + c4
